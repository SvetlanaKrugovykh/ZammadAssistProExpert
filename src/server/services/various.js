...
function fDateTime(local, time = new Date()) {
  const DELTA_RUBY_TIME_FOR_MESSAGE_MINUTES = Number(process.env.DELTA_RUBY_TIME_FOR_MESSAGE_MINUTES) || 0
  let adjustedTime = new Date(time)

  if (isNaN(adjustedTime)) {
    adjustedTime = new Date()
  }
  adjustedTime = new Date(adjustedTime.getTime() - DELTA_RUBY_TIME_FOR_MESSAGE_MINUTES * 60000)
